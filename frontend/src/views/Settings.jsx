...
export default function Settings() {
  ...
  const doImport = ev => {
    ...
    rd.onload = () => {
      try {
        ...
        if (!data.workouts || !data.routines) throw new Error('not a Cospel backup')
  ...
  return <div className="narrow">
    ...
    <Section title={MOBILE ? (user ? t('Your server') : t('Your data')) : DEMO ? t('Demo') : t('Account')}>
      {MOBILE ? (user ? <>
        <Row icon="personCircle" iconTint="var(--grey)" title={user.name} subtitle={t('Synced with your Cospel server.')} />
        ...
        <Row icon="link" iconTint="var(--indigo)" title={t('Connect to my server')} subtitle={t('Sync this device to your own self-hosted Cospel instead.')} accessory="chevron"
        ...
        <Row icon="rocket" iconTint="var(--indigo)" title={t('Self-host Cospel')} subtitle={t('Passkey sign-in, sync across your devices, your own data.')} accessory="chevron"
          onClick={() => window.open(REPO, '_blank', 'noopener')} />
      </> : user ? <>
        <Row icon="personCircle" iconTint="var(--grey)" title={user.name} subtitle={t('Signed in with passkey — data syncs to this profile.')} />
        {user.admin && <Row icon="wrench" iconTint="var(--indigo)" title={t('Admin dashboard')} accessory="chevron" onClick={() => nav('/admin')} />}
        <Row icon="link" iconTint="var(--blue)" title={t('Pair the mobile app')} subtitle={t('Connect the Cospel app on your phone to this account.')} accessory="chevron"
    ...
    {!MOBILE && <Section title={t('Tip')}>
      <Row icon="lightbulb" iconTint="var(--yellow)"
        ...
        subtitle={t('to install Cospel as a full-screen app.') + ' ' + (user ? t('Your data syncs with your profile — sign in anywhere to see it.') : t('Guest data stays on this device — export a backup now and then!'))} />
    </Section>}
    ...
    <div className="dim small" style={{ textAlign: 'center', marginTop: 4, lineHeight: 1.6 }}>
      Cospel v{__APP_VERSION__} · {t('free & open source (AGPL v3)')}<br />
      <a href="https://github.com/Jandresdiaz87/opengym" target="_blank" rel="noopener">source code</a> · exercise data: hasaneyldrm/exercises-dataset (MIT)<br />
  ...
  return <>
    <Section
      ...
      footer={on && S.reminder?.on
        ...
        : null}
    >
      <Row icon="bell" iconTint="var(--red)" title={t('Push notifications')} subtitle={t('Rest-timer alerts, even if Cospel is closed.')}>
...
function PairSheet({ close }) {
  ...
  return <>
    ...
    <div className="muted small" style={{ marginBottom: 14 }}>
      {t('On the Cospel app, choose “Connect to my server”, then enter this address and the code below. It expires in 5 minutes.')}
    </div>
